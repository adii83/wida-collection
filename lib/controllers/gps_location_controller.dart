...
class GpsLocationController extends GetxController {
  ...
  void startRecording() {
    ...
    if (kDebugMode) {
      print('🧪 [GPS] Recording started');
    }

    Get.snackbar(
      'Recording dimulai',
      'Aplikasi sedang merekam data GPS',
  ...
  Future<void> _exportCsv() async {
    try {
      ...
      final filename =
          'gps_experiment_${DateTime.now().millisecondsSinceEpoch}.csv';

      // Catatan: path ini khusus Android (folder Download)
      ...
      if (kDebugMode) {
        print('🧪 [GPS] CSV exported to ${file.path}');
      }
    } catch (e) {
      Get.snackbar(
        'Gagal export CSV',
      ...
      if (kDebugMode) {
        print('❌ [GPS] CSV export error: $e');
      }
    }
  }

  void _logExperiment(Position pos) {
    if (!_isRecording.value) return; // tidak merekam → abaikan
    ...
    if (kDebugMode) {
      print(
        '🧪 [GPS] log -> acc: ${pos.accuracy}, delay: $delay, dist: $distance, speed: $speed',
      );
    }
  }
